 ...
 workflowData.states.forEach(state => {
  ...
  if (state.end) {
   ...
   const endNode = {
    ...
    type: 'end',
    position: {
     x: (stateNodeMap[state.name] ? nodes.find(n => n.id === stateNodeMap[state.name])?.position.x : 0) + 300,
     y: (stateNodeMap[state.name] ? nodes.find(n => n.id === stateNodeMap[state.name])?.position.y : 0)
    },
    data: { label: 'End' }
   };
   nodes.push(endNode);
